...
########################################################################################################################
"""                                              Налаштування бота і файлів                                          """
########################################################################################################################

bot = telebot.TeleBot('ENTER YPU TOKEN HERE')
...
@bot.message_handler(commands=['start'])  # якщо бот отримує команду "/start"
def start(message):
    mess = f'''
...
@bot.message_handler(commands=['help'])  # якщо бот отримує команду "/help"
def help(message):
    commands = f'''
...
* Надішли комусь "+" або "-", щоб змінити рейтинг юзера.

* Якщо ти хочешь дізнатися топ рейтинг усіх юзерів, то напиши "топ рейтинг".
'''
...
5. {top5[4][1]} — {top5[4][0]}."""
    ...
    x = random.randint(0, 50)  # роляємо рандомно одне число, якщо отримуємо 3, то бот відправляє стикер
    if x == 3:
